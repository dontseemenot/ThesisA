...
while 1
    ...
    if numel(tline)>13
        if tline(1:14) == 'Recording Date'
            labelledStartDate = tline(17:end)
        end
    end
end
...
labelledStartTime = strcat(num2str(start_time.h), '.', num2str(start_time.m), '.', num2str(start_time.s));
labelledStartDateTime = sprintf('%s %s', labelledStartDate, labelledStartTime);
clear labelledStartDate; clear labelledStartTime;